...
fs.readFile('old-skins.json', 'utf8', (err, data) => {
    ...
    const pistols = {};  // Changed from rifles to pistols
    ...
    // Iterate through each skin and filter pistols
    for (const skinId in skins) {
        const skin = skins[skinId];

        // Check if the category is "Pistols" (changed from Rifles)
        if (skin.category && skin.category.name === "Pistols") {
            ...
            if (!pistols[baseId]) {
                pistols[baseId] = {
                    ...
                    category: "pistol",  // Changed from rifle to pistol
            ...
            if (!pistols[baseId].wears.includes(skin.wear.name)) {
                pistols[baseId].wears.push(skin.wear.name);
            }
            pistols[baseId].image[skin.wear.name] = skin.image;

            // Check if StatTrak or Souvenir is true for any variant
            if (skin.stattrak) {
                pistols[baseId].stattrak = true;
            }
            if (skin.souvenir) {
                pistols[baseId].souvenir = true;
            }

            // Add buff and stattrak IDs for each wear state (if available)
            if (skin[`buff-id-${skin.wear.name.toLowerCase().replace(/ /g, '-')}`]) {
                pistols[baseId][`buff-id-${skin.wear.name.toLowerCase().replace(/ /g, '-')}`] = skin[`buff-id-${skin.wear.name.toLowerCase().replace(/ /g, '-')}`];
            }

            if (skin[`stattrak-id-${skin.wear.name.toLowerCase().replace(/ /g, '-')}`]) {
                pistols[baseId][`stattrak-id-${skin.wear.name.toLowerCase().replace(/ /g, '-')}`] = skin[`stattrak-id-${skin.wear.name.toLowerCase().replace(/ /g, '-')}`];
            }
                ...
                if (collection.contains.some(item => item.id === baseId)) {
                    ...
                    if (collection.crates && collection.crates.length > 0) {
                        collection.crates.forEach(crate => {
                            ...
                            if (!pistols[baseId].crates.some(existingCrate => existingCrate.id === crate.id)) {
                                pistols[baseId].crates.push({
                                    id: crate.id,
                                    name: crate.name,
                                    image: crate.image
                                });
                    ...
                    if (collection.crates.length === 0) {
                        ...
                        if (!pistols[baseId].collections.some(existingCollection => existingCollection.id === collection.id)) {
                            pistols[baseId].collections.push({
                                id: collection.id,
    ...
    // Convert the pistols object into an array for saving as JSON
    const pistolArray = Object.values(pistols);

    // Write the merged pistol skins to a new pistols.json file
    fs.writeFile('pistols.json', JSON.stringify(pistolArray, null, 4), (err) => {
        if (err) {
            console.error("Error writing the file:", err);
            return;
        }

        console.log(`Extracted and merged ${pistolArray.length} pistol skins with collections and crates.`);
    });
});
